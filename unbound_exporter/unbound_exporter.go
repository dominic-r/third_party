...
func (e *UnboundExporter) Collect(ch chan<- prometheus.Metric) {
	...
	if err == nil {
		...
		_ = level.Error(logger).Log("Failed to scrape socket: ", err)
		ch <- prometheus.MustNewConstMetric(
			unboundUpDesc,
			prometheus.GaugeValue,
			0.0)
...
func main() {
	...
	_ = level.Info(logger).Log("Starting unbound_exporter")
	...
	_ = level.Info(logger).Log("Listening on address:port => ", *listenAddress)
